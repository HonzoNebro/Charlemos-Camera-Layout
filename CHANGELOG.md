...
### Fixed
- The on-screen editor now derives its target from the active section, so Marco / imagen superpuesta edits the frame and Nombre edits the name instead of camera geometry.

### Prerelease
- Opt-in UX correction on `preview/ux-editor`, with matching `v3.3.0-beta.10` archive. Stable distribution remains unchanged.
...
### Changed
- Foundry VTT compatibility now supports V13 through V14:
  - minimum version: `13`
  - verified version: `14`
- Release metadata updated for stable distribution:
  - module version: `3.0.2`
  - download URL points to tag archive `v3.0.2`
...
- Release metadata updated for stable distribution:
  ...
  - download URL points to tag archive `v3.0.1`
